...
def find_term_in_literals(query_term:str, lang:str) -> list:
    ...
    gzip_path = f"/AAT/gzip_aat_subgraph_{lang}.json"
...
def get_bows(lang:str) -> dict:
    ...
    path_to_results = f"/AAT/aat_query_results_{lang}.json"
...
def get_lit_related_matches_bow(lang:str) -> dict:
    ...
    the info about which AAT concept is a related match to the query term is taken from /related_matches/rm.csv
    lang: str, 'en' or 'nl'
    ...
    # load aat bows
    path_to_bows = f"/AAT/aat_bows_{lang}.json"
...
def get_cs(lang:str):
    ...
    # load bckground info
    path_bg = "/bg/background_info_bows.json"
    bg_info = requests.get(path_bg).json()

    # load aat bows

    path_aat_bows = f"/AAT/aat_bows_{lang}.json"
